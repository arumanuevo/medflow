...
if (strpos($c, 'id="contacto"') === false) {
    if (strpos($c, '<footer class="footer mt-5 pt-4">') !== false) {
        $c = str_replace('<footer class="footer mt-5 pt-4">', $contactHtml, $c);
        file_put_contents('resources/views/landing.blade.php', $c);
        echo "Contact form successfully injected!\n";
    } else {
        echo "Error: Footer tag not found. Available structure around end:\n";
        echo substr($c, strlen($c) - 1000);
    }
} else {
    echo "Contact form already exists!\n";
}
