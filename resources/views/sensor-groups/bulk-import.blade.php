            ...
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <div id="step1" class="step">
                        ...
                        <p class="text-muted">Selecciona el grupo de sensores y sube un archivo <strong>.xlsx</strong> o <strong>.csv</strong> con los datos.</p>
                        ...
                        <div class="alert alert-info">
                            ...
                            <ul class="mb-0 mt-2">
                                ...
                                <li>El archivo debe contener una columna con el <strong>identificador</strong> (único).</li>
                                <li>La <strong>descripción</strong> es opcional.</li>
                                <li>Puedes agregar <strong>campos adicionales</strong> (Lote, Apellido, Ubicación, etc.) en el paso 2.</li>
                            </ul>
                        </div>
                        ...
                        <div id="templateInfo" class="alert alert-light d-none">
                            <h6><i class="bi bi-file-text"></i> Plantilla del grupo: <span id="templateNameDisplay"></span></h6>
                            <p class="mb-0"><strong>Campos de medición de este grupo:</strong></p>
                            <ul id="templateFieldsList" class="mb-0">
                                <li class="text-muted"><i class="bi bi-info-circle"></i> Estos campos se usan al tomar mediciones, no al importar sensores.</li>
                            </ul>
                        </div>
                    ...
                    <div id="step2" class="step d-none">
                        ...
                        <div class="alert alert-warning border-2 border-warning mb-3">
                            <div class="d-flex align-items-start">
                                ...
                                <div>
                                    <h6 class="alert-heading mb-1">🔑 Campo Clave: Identificador</h6>
                                    <p class="mb-1 small">
                                        El campo <strong>"Identificador"</strong> es la <strong>clave única</strong> del sensor.
                                    </p>
                                    <ul class="small mb-0">
                                        <li>
                                            <span class="text-success">✅</span> 
                                            Si el <strong>Identificador NO existe</strong> → Se <strong>CREA</strong> un nuevo sensor
                                        </li>
                                        <li>
                                            <span class="text-warning">🔄</span> 
                                            Si el <strong>Identificador YA existe</strong> → Se <strong>SOBRESCRIBE</strong> el sensor existente
                                        </li>
                                    </ul>
                    ...
                    <div id="step3" class="step d-none">
                        ...
                        <div class="alert alert-warning mb-3">
                            ...
                            <ul class="mb-0 small">
                                <li>
                                    <strong>Identificador único:</strong> El campo <strong>"Identificador"</strong> se usa como clave única.
                                </li>
                                <li>
                                    <strong>Sobrescritura:</strong> Si un sensor con el mismo <strong>Identificador</strong> ya existe en el grupo:
                                    <ul>
                                        <li>✅ Se actualizarán: <strong>Nombre</strong> y <strong>Descripción</strong></li>
                                        <li>✅ Se actualizarán: <strong>Campos Adicionales</strong> (Lote, Apellido, etc.)</li>
                                        <li>❌ <strong>No se modificarán:</strong> Mediciones históricas, Fecha de creación, Estado de marcado</li>
                                    </ul>
                                </li>
                                <li>
                                    <strong>Creación:</strong> Si el <strong>Identificador</strong> no existe, se creará un nuevo sensor.
                                </li>
                            </ul>
                        </div>
                        ...
                        <div id="importSummary" class="import-summary d-none">
                            ...
                            <ul>
                                <li><strong>Total de registros en el archivo:</strong> <span id="totalRecords">0</span></li>
                                <li><strong>Registros válidos (en preview):</strong> <span id="validRecords">0</span></li>
                                <li><strong>Registros con errores (en preview):</strong> <span id="errorRecords">0</span></li>
                            </ul>
                        </div>

                        <div id="importErrors" class="error-container d-none">
                            <h5><i class="bi bi-exclamation-triangle btn-icon"></i> Errores Encontrados en el Preview</h5>
                            <ul id="errorList"></ul>
                        </div>
                    </div>
...
/**
 * Cargar campos extras disponibles
 */
function loadExtraFields() {
    ...
    // ✅ Verificar token
    if (!token) {
        console.warn('No hay token, mostrando campos extras por defecto');
        // Mostrar campos extras por defecto aunque no haya token
        extraFields = [
    ...
    $.ajax({
        url: '/api/sensor-groups/extra-fields',
        type: 'GET',
        headers: {
            'Authorization': 'Bearer ' + token,
            'Accept': 'application/json'
        ...
        success: function(response) {
            console.log('✅ Campos extras cargados:', response);
            if (response.success) {
        ...
        error: function(xhr) {
            console.error('❌ Error al cargar campos extras:', xhr);
            // ✅ Mostrar campos por defecto aunque falle la API
            extraFields = [
                { name: 'lote', label: 'Lote' },
                { name: 'apellido', label: 'Apellido' },
                { name: 'ubicacion', label: 'Ubicación' },
            ...
            renderExtraFields();
            showAlert('No se pudieron cargar campos adicionales predefinidos. Puedes agregarlos manualmente.', 'warning');
        }
    });
}
    ...
    const html = `
        <div class="card mt-3">
            <div class="card-header bg-light d-flex justify-content-between align-items-center">
                <h6><i class="bi bi-plus-circle"></i> Campos Adicionales (Opcional)</h6>
            ...
            <div class="card-body">
                <p class="text-muted small">
                    <i class="bi bi-info-circle"></i> 
                    Selecciona columnas adicionales para guardar como metadatos del sensor.
                    Estos campos se almacenarán en la base de datos como información adicional.
                </p>
