...
@section('content')
    ...
    ============================================ -->
                ...
                @endphp
                @if(isset($previousMeasurement) && $previousMeasurement)
                    <div class="alert alert-{{ $estadoPeriodo === 'ok' ? 'success' : ($estadoPeriodo === 'temprano' ? 'warning' : ($estadoPeriodo === 'vencido' ? 'danger' : 'warning')) }} mb-3"
                        style="font-size: 0.85rem; padding: 0.5rem 0.75rem;">
                        <i
                            class="bi bi-{{ $estadoPeriodo === 'ok' ? 'check-circle' : ($estadoPeriodo === 'temprano' ? 'info-circle' : ($estadoPeriodo === 'vencido' ? 'exclamation-triangle' : 'clock')) }} me-2"></i>
                        {!! $mensajePeriodo !!}
                    </div>
                @endif
                <!-- ============================================
                ...
                <form id="measurementForm">
                    ...
                    <input type="hidden" id="groupName"
                        value="{{ str_replace(' ', '_', $sensor->group->name ?? 'SinGrupo') }}">
                    ...
                    <div class="mb-3">
                        ...
                        <input type="datetime-local" class="form-control" id="fecha_medicion" name="data[fecha_medicion]"
                            required>
                        <small class="text-muted">
                            <i class="bi bi-info-circle me-1"></i>
                            ...
                            @foreach($customFields as $campo)
                                <div class="col-md-6">
                                    ...
                                    @if($campo['tipo'] === 'numero')
                                        <input type="number" step="0.01" class="form-control" id="field_{{ $campo['nombre'] }}"
                                            name="data[campos_personalizados][{{ $campo['nombre'] }}]"
                                            placeholder="Ingresa {{ $campo['nombre'] }}">
                                    @elseif($campo['tipo'] === 'texto')
                                        <input type="text" class="form-control" id="field_{{ $campo['nombre'] }}"
                                            name="data[campos_personalizados][{{ $campo['nombre'] }}]"
                                            placeholder="Ingresa {{ $campo['nombre'] }}">
                                    @elseif($campo['tipo'] === 'fecha')
                                        <input type="date" class="form-control" id="field_{{ $campo['nombre'] }}"
                                            name="data[campos_personalizados][{{ $campo['nombre'] }}]">
